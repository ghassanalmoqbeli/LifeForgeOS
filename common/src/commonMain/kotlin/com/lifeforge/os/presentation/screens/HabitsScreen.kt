...
@Composable
fun HabitsScreen(viewModel: HabitsViewModel, strings: L10n) {
    ...
    Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
        Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.fillMaxWidth()) {
            OutlinedTextField(
                ...
                label = { Text(strings.habitNameHint) },
                singleLine = true,
        ...
        if (state.isLoading) {
            ...
            Text(strings.noHabits, style = MaterialTheme.typography.bodyMedium)
        } else {
            LazyColumn(modifier = Modifier.fillMaxSize()) {
                items(state.habits, key = { it.id }) { habit ->
                    Row(
                        ...
                        Checkbox(
                            checked = state.completedToday.contains(habit.id),
