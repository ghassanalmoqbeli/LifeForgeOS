...
class AppLockManagerImpl(
    ...
    override fun unlockWithBiometric(callback: (Boolean, String?) -> Unit) {
        biometric.authenticate("Unlock LifeForge OS", "Authenticate to access your data") { success, error ->
            if (success) {
                _isLocked.value = false
                _lockReason.value = LockReason.Unlocked
                lastActiveTime = System.currentTimeMillis()
    ...
    private fun checkAutoLock() {
        if (!_isLocked.value && System.currentTimeMillis() - lastActiveTime >= cachedTimeoutMs) {
